...
### 필터링 요소
| 1. 기간 필터       | 2. 페이 타입     |  3. 결제 상태     |
|  :------------: |  :------------: | :------------: |
| 사용자 선택 가능 기간 (주별 / 월별 / 연도별)  | 각 수단별로 얼마나 결제했는가, 그리고 퍼센트 보여주기 |결제 상태 분포 /성공률 |

### 화면 구성
- **총 매출 금액, 거래 건수** : 크게 표시하여 한눈에 정보를 알 수 있도록 했습니다.
- **Nivo 커스텀 그래프** :막대 증감 그래프, 결제 수단, 상태 현황 그래프
- **tailwind 로 구현한 결제수단 % 막대 그래프** : 원형 그래프와 퍼센트 그래프를 함께 한눈에 볼 수 있게 구현했습니다.
- **daisyUI**를 이용한 필터링 버튼 : 기간선택(주별/월별/연도별) 비교 가능, 통화 선택 (현재는 한국 돈만 선택 가능, 해외 결제 대비)
...
그로 인해 구현하고 싶었던 기능들을 모두 완성하지 못한 점이 아쉬웠습니다.  

다만 제한된 시간 속에서도 데이터를 직접 분석하며 화면에 필요한 요소를 정의하는 과정에서 많은 것을 배울 수 있었습니다.
특히 결제/핀테크 개발에서 데이터의 의미를 먼저 정확히 해석하고, 요구되는 통계를 어떤 기준으로 산출해야 하는지 판단하며,
또 월·주·일 단위의 필터링 규칙을 스스로 설정해가는 과정을 통해 전자금융 서비스 개발에서 요구되는 사고방식을 경험할 수 있었습니다.

이번 과정을 통해 결제 도메인에 대한 이해를 쌓을 수 있었고, 데이터 기반으로 기능을 설계하는 역량을 강화할 수 있었다는 점에서 의미 있는 경험이었습니다.  
감사합니다.

### 더 수정하고 싶은점
- [거래 내역 전체 조회] 를 zustand에 저장해서 한번만 호출해도 되도록 수정하기
- 거래내역 리스트 조회에서 가맹점 상태별, 결제 수단별, 거래금액순, 카테고리별(biz)로 필터링, 검색기능 추가
